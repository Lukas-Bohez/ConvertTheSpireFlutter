...
GraphicsContext::GraphicsContext(rx::RoHelper* rohelper) : rohelper_(rohelper) {
  ...
  if (!device_) {
    CtsProbeLog("gfx: CreateD3DDevice returned NULL");
    return;
  }
  CtsProbeLog("gfx: D3D11 device created");
  ...
  if (FAILED(util::CreateDirect3D11DeviceFromDXGIDevice(
    ...
    CtsProbeLog("gfx: CreateDirect3D11DeviceFromDXGIDevice FAILED");
    return;
  }
  CtsProbeLog("gfx: WinRT Direct3D11 device interop ok");
