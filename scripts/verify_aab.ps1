...
$aab = Resolve-Path "c:\\development\\ConversionFlutter\\my_flutter_app\\aab\\convert-the-spire-reborn-v12.1.4+1214-play-release.aab"
...
try {
  ...
  $decoded = @($_.Exception.Message)
}
if (-not $decoded -or (($decoded | Out-String) -match "error")) {
...
$touchFalse= ($decoded | Out-String) -match "android.hardware.touchscreen" -and
             (($decoded | Where-Object { $_ -match "touchscreen" } | Select-Object -First 5) -join "`n") -match "0x00000000"
